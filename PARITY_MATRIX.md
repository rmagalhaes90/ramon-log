...
| Onboarding                        |       ✅ |  🟡 | Preferência inicial persistida, tour guiado dos quatro módulos (Train/Recover/Fuel/Sync) exibido uma vez por conta, e spotlight interativo no dashboard convidando quem ainda não tem rotina a criar a primeira; falta validação visual em dispositivo real                         |
| Treinos/rotinas/exercícios/séries |       ✅ |  🟡 | Telas de Treino (execução) e Rotina (montagem/edição) separadas como no baseline; vídeo do YouTube em modal embutido (180 links portados, incluindo 10 exercícios de cardio novos); alternativas de exercício em acordeon recolhido; edição de séries/reps/descanso movida para a tela de Rotina; celebração com confete/sequência ao finalizar; gerador automático por grupo muscular/equipamento/intensidade (agora sugerindo intensidade pelo objetivo do perfil) somado a seis templates fixos (Full Body, Upper/Lower, PPL, PPL+Upper/Lower, Bro Split, Full Body 5x), cada um com exercícios diversificados entre dias repetidos; cronômetro da sessão só inicia por ação do usuário (nunca ao abrir a tela) e dias/semana sem rotina mostram onboarding em vez de tela vazia contando tempo; séries, notas, timers com pausa, descanso cancelável, aquecimento, anilhas, e1RM, PR e rascunho recuperável; drag-and-drop por toque em iOS/Android segue pendente |
...
| Nutrição/barcode/suplementos      |       ✅ |  🟡 | Refeições, macros, fibra, água, favoritos, cópia entre dias, suplementos, consulta GTIN e leitor de código de barras via ZXing (funciona em qualquer navegador, inclusive Safari/iOS, carregado sob demanda) com fallback manual                                                     |
...
| PWA/atualização                   |       ✅ |  🟡 | Worker versionado e cache runtime de chunks; aviso de atualização agora recarrega de fato (corrigido `clients.claim()` ausente que travava o aviso na tela); faltam rollback e suspensão iOS |
...
| Admin                             |       ✅ |  🟡 | Listagem, bloqueio e concessão/revogação de admin por custom claims aprovados no Emulator; gerenciador de exercícios compartilhado (Firestore, editável sem deploy, vídeo PT/EN) e novo papel Coach (custom claim, vínculo aluno↔treinador por código de convite, escopo de acesso testado no Emulator) — Coach não existe no baseline, é capacidade nova |
...
| Treinos, rotinas, catálogo e histórico         |       97% | Núcleo, templates, gerador automático (grupo muscular/equipamento/intensidade), reorder, edição de alvo por exercício, rascunhos, PR/e1RM, 170 vídeos reais, relatório compartilhável e conquistas implementados |
