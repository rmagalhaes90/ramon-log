...
import { enqueuePhoto, flushPhotoUploads, photoQueueCount } from './features/photos/offline';
...
import { sessionsCsv } from './features/backup/csv';
...
function shell(content: string): void {
  activeCameraStop?.();activeCameraStop=undefined;
...
function renderDashboard(user: User): void {
  ...
  void Promise.all([flushUserDataQueue(user),flushPhotoUploads(user)]).catch((error: unknown) => reportError(error, 'sync/flush'));
  void Promise.all([queueList(),photoQueueCount(user)]).then(([items,photos]) => { const count = document.querySelector('#queue-count'); if (count) count.textContent = String(items.length+photos); })
...
async function renderSettings(user: User): Promise<void> {
  ...
  const csvButton=document.createElement('button');csvButton.textContent=copy('exportCsv');document.querySelector('.backup-card>div')?.append(csvButton);csvButton.addEventListener('click',()=>void loadUserData(user,'sessionLog').then((sessions)=>{const blob=new Blob([sessionsCsv(sessions??[])],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`kyro-sessions-${dateKey()}.csv`;link.click();URL.revokeObjectURL(url);}).catch((error:unknown)=>reportError(error,'backup/csv')));
...
async function renderProgress(user: User): Promise<void> {
  const [weights, measurements, readiness, sessions] = await Promise.all([
    loadUserData(user, 'bodyWeights').then((value) => value ?? []), loadUserData(user,'bodyMeasurements').then((value)=>value??{}), loadUserData(user, 'readinessLog').then((value) => value ?? {}), loadUserData(user, 'sessionLog').then((value) => value ?? []),
  ]);
  ...
  const report=weeklyReport(sessions);const streak=trainingStreak(sessions);const achievements=unlockedAchievements(sessions.length,sessions.reduce((sum,item)=>sum+item.volume,0),streak);
  ...
  const achievementList=document.querySelector('#achievement-list');if(!achievements.length){const empty=document.createElement('span');empty.textContent=copy('noAchievements');achievementList?.append(empty);}achievements.forEach((key)=>{const badge=document.createElement('span');badge.textContent=copy(`achievement_${key}` as MessageKey);achievementList?.append(badge);});
  drawProgressChart(document.querySelector('#weight-chart'),weights.map((item)=>({d:item.d,value:item.kg})));
  bindBack(user);
  ...
  document.querySelector('#measurements-form')?.addEventListener('submit',(event)=>{event.preventDefault();const data=new FormData(event.currentTarget as HTMLFormElement);const entry=Object.fromEntries(['waist','chest','arm','hip','thigh'].flatMap((key)=>{const value=Number(data.get(key));return Number.isFinite(value)&&value>0?[[key,value]]:[]}));if(!Object.keys(entry).length)return;void saveUserData(user,'bodyMeasurements',{...measurements,[dateKey()]:entry}).then(()=>renderProgress(user)).catch((error:unknown)=>reportError(error,'progress/measurements'));});
...
function drawProgressChart(container:Element|null,values:{d:string;value:number}[]):void{if(!container)return;const points=chartPoints(values);if(!points.length){container.textContent=copy('noChartData');return;}const ns='http://www.w3.org/2000/svg';const svg=document.createElementNS(ns,'svg');svg.setAttribute('viewBox','0 0 600 180');svg.setAttribute('role','img');const line=document.createElementNS(ns,'polyline');line.setAttribute('points',points.map(({x,y})=>`${x},${y}`).join(' '));line.setAttribute('fill','none');line.setAttribute('stroke','currentColor');line.setAttribute('stroke-width','5');line.setAttribute('vector-effect','non-scaling-stroke');svg.append(line);for(const point of points){const circle=document.createElementNS(ns,'circle');circle.setAttribute('cx',String(point.x));circle.setAttribute('cy',String(point.y));circle.setAttribute('r','6');const title=document.createElementNS(ns,'title');title.textContent=`${point.d}: ${point.value}`;circle.append(title);svg.append(circle);}container.replaceChildren(svg);}

async function renderPhotos(user:User):Promise<void>{
  const [photos,pendingCount]=await Promise.all([loadUserData(user,'photoIndex').then((value)=>value??[]),photoQueueCount(user)]);const selected=new Set<string>();
  ...
  document.querySelector('#photo-form')?.addEventListener('submit',(event)=>{event.preventDefault();const input=document.querySelector<HTMLInputElement>('#photo-input');const file=input?.files?.[0];if(!file)return;const error=validatePhoto(file);const status=document.querySelector('#photo-status');if(error){if(status)status.textContent=copy(error);return;}const progress=document.querySelector<HTMLProgressElement>('#photo-progress');if(progress)progress.hidden=false;const id=crypto.randomUUID().slice(0,60);const run=navigator.onLine?uploadPhoto(user,id,file,(value)=>{if(progress)progress.value=value;}).then(()=>saveUserData(user,'photoIndex',[...photos,{id,d:dateKey()}])):enqueuePhoto(user,id,file);void run.then(()=>renderPhotos(user)).catch(async(error:unknown)=>{if(!navigator.onLine){try{await enqueuePhoto(user,id,file);await renderPhotos(user);return;}catch(queueError){error=queueError;}}try{await deletePhoto(user,id);}catch{ /* Upload may not have created an object. */ }reportError(error,'photos/upload');if(status)status.textContent=copy((error as Error).message==='photoQueueFull'?'photoQueueFull':'uploadFailed');});});
}
...
async function renderNutrition(user: User): Promise<void> {
  ...
  document.querySelector('#barcode-form')?.addEventListener('submit',(event)=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;const code=formText(new FormData(form),'barcode');const status=document.querySelector('#barcode-status');if(!navigator.onLine){if(status)status.textContent=copy('barcodeOffline');return;}if(status)status.textContent=copy('searching');const controller=new AbortController();const timeout=window.setTimeout(()=>controller.abort(),8000);void lookupBarcode(code,controller.signal).then((food)=>{if(!food){if(status)status.textContent=copy('barcodeNotFound');return;}const mealForm=document.querySelector<HTMLFormElement>('#meal-form');const values:{name:string;kcal:number;protein:number;carb:number;fat:number}=food;Object.entries(values).forEach(([key,value])=>{const input=mealForm?.elements.namedItem(key);if(input instanceof HTMLInputElement)input.value=String(value);});if(status)status.textContent=copy('barcodeReady');document.querySelector<HTMLInputElement>('#meal-form input[name="name"]')?.focus();}).catch((error:unknown)=>{if((error as Error).name==='AbortError'){if(status)status.textContent=copy('barcodeTimeout');return;}reportError(error,'nutrition/barcode');if(status)status.textContent=copy((error as Error).message==='barcodeInvalid'?'barcodeInvalid':'barcodeUnavailable');}).finally(()=>window.clearTimeout(timeout));});
  document.querySelector('#scan-barcode')?.addEventListener('click',()=>{const video=document.querySelector<HTMLVideoElement>('#barcode-video');const form=document.querySelector<HTMLFormElement>('#barcode-form');const input=form?.elements.namedItem('barcode');const status=document.querySelector('#barcode-status');if(!video||!(input instanceof HTMLInputElement))return;video.hidden=false;if(status)status.textContent=copy('cameraStarting');void startBarcodeCamera(video,(code)=>{activeCameraStop=undefined;video.hidden=true;input.value=code;form?.requestSubmit();}).then((stop)=>{activeCameraStop=stop;}).catch((error:unknown)=>{video.hidden=true;reportError(error,'nutrition/camera');if(status)status.textContent=copy('cameraDenied');});});
...
async function renderWorkout(user: User): Promise<void> {
  clearWorkoutTimers();
  const [workoutsValue,draft]=await Promise.all([loadUserData(user,'workouts'),loadWorkoutDraft(user,selectedDay)]);const workouts=workoutsValue??{};const freshEntries=createEntries(workouts,selectedDay);const draftMatches=draft&&draft.entries.map((item)=>item.exercise.name).join('\n')===freshEntries.map((item)=>item.exercise.name).join('\n');workoutEntries=draftMatches?draft.entries:freshEntries;if(draftMatches)workoutStartedAt=draft.startedAt;
  ...
  document.querySelector('#routine-template')?.addEventListener('click',()=>renderTemplatePicker(user,workouts));
...
function renderTemplatePicker(user:User,workouts:Workouts):void{shell(`<section class="feature-view"><button id="template-back" class="link-button">← ${copy('back')}</button><p class="eyebrow">KYRO BUILDER</p><h1>${copy('chooseTemplate')}</h1><div id="template-list" class="template-list"></div></section>`);document.querySelector('#template-back')?.addEventListener('click',()=>void renderWorkout(user));const list=document.querySelector('#template-list');(['fullbody','upperLower','ppl'] as TemplateKey[]).forEach((key)=>{const card=document.createElement('article');const title=document.createElement('h2');title.textContent=copy(key);const description=document.createElement('p');description.textContent=copy(`${key}Description` as MessageKey);const apply=document.createElement('button');apply.textContent=copy('applyTemplate');apply.addEventListener('click',()=>{if(!confirm(copy('templateConfirm')))return;const generated=createTemplate(key);void saveUserData(user,'workouts',{...workouts,...generated}).then(()=>Promise.all(Object.keys(generated).map((day)=>clearWorkoutDraft(user,day as DayKey)))).then(()=>{selectedDay=(Object.keys(generated)[0] as DayKey|undefined)??'segunda';workoutStartedAt=new Date().toISOString();return renderWorkout(user);}).catch((error:unknown)=>reportError(error,'workout/template'));});card.append(title,description,apply);list?.append(card);});}
...
function renderExerciseEntries(user:User,workouts:Workouts): void {
  ...
  const persistDraft=()=>void saveWorkoutDraft(user,selectedDay,workoutStartedAt,workoutEntries).catch((error:unknown)=>reportError(error,'workout/draft'));
  ...
  workoutEntries.forEach((entry, exerciseIndex) => {
    ...
    const top=document.createElement('div');top.className='exercise-top';const heading = document.createElement('h2'); heading.textContent = entry.exercise.name;const actions=document.createElement('div');const reorder=(direction:-1|1)=>void saveUserData(user,'workouts',moveExercise(workouts,selectedDay,exerciseIndex,direction)).then(()=>clearWorkoutDraft(user,selectedDay)).then(()=>renderWorkout(user)).catch((error:unknown)=>reportError(error,'workout/reorder'));const up=document.createElement('button');up.textContent='↑';up.ariaLabel=copy('moveUp');up.disabled=exerciseIndex===0;up.addEventListener('click',()=>reorder(-1));const down=document.createElement('button');down.textContent='↓';down.ariaLabel=copy('moveDown');down.disabled=exerciseIndex===workoutEntries.length-1;down.addEventListener('click',()=>reorder(1));const remove=document.createElement('button');remove.textContent=copy('remove');remove.addEventListener('click',()=>{const current=workouts[selectedDay];if(!current)return;const nextExercises=current.exercises.filter((_,index)=>index!==exerciseIndex);void saveUserData(user,'workouts',{...workouts,[selectedDay]:{...current,exercises:nextExercises}}).then(()=>clearWorkoutDraft(user,selectedDay)).then(()=>renderWorkout(user)).catch((error:unknown)=>reportError(error,'workout/remove'));});actions.append(up,down,remove);top.append(heading,actions);card.append(top);
    ...
    entry.sets.forEach((set, setIndex) => {
      ...
      const guidance=document.createElement('small');guidance.className='set-guidance';kg.value=set.kg?String(set.kg):'';reps.value=set.reps?String(set.reps):'';done.checked=set.done;row.classList.toggle('done',set.done);kg.addEventListener('input', () => { set.kg = Number(kg.value) || 0;const plates=calculatePlates(set.kg);guidance.textContent=set.kg>20?`${copy('warmup')} ${Math.round(set.kg*.5)}kg · ${copy('plates')}: ${plates.join(' + ')||'—'}`:'';persistDraft(); }); reps.addEventListener('input', () => { set.reps = Number(reps.value) || 0;persistDraft(); }); done.addEventListener('change', () => { set.done = done.checked; row.classList.toggle('done', done.checked);persistDraft(); });
