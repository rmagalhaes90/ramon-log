...
export function bestCompletedSet(entry:ExerciseEntry):{maxWeight:number;maxWeightReps:number;maxE1rm:number}|null{
  const sets=entry.sets.filter((set)=>set.done&&set.kg>0&&set.reps>0);if(!sets.length)return null;
  const heaviest=[...sets].sort((a,b)=>b.kg-a.kg||b.reps-a.reps)[0];
  return heaviest?{maxWeight:heaviest.kg,maxWeightReps:heaviest.reps,maxE1rm:Math.max(...sets.map((set)=>estimatedOneRepMax(set.kg,set.reps)))}:null;
}

export function calculatePlates(targetKg:number,barKg=20,available=[25,20,15,10,5,2.5,1.25]):number[]{
  if(!Number.isFinite(targetKg)||targetKg<=barKg)return[];let side=(targetKg-barKg)/2;const plates:number[]=[];
  for(const plate of available){while(side+1e-9>=plate){plates.push(plate);side-=plate;}}
  return plates;
}
