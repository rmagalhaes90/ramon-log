...
const draftSchema = z.object({
  ...
  entries: z
    .array(z.object({ exercise: exerciseSchema, sets: z.array(draftSetSchema).max(20) }))
    .max(90),
});
export type WorkoutDraft = z.infer<typeof draftSchema>;
const key = (uid: string, day: DayKey) => `workout-draft:${uid}:${day}`;
export function parseWorkoutDraft(value: unknown): WorkoutDraft {
...
export async function loadWorkoutDraft(
  ...
  const value = await cacheGet<unknown>(key(user.uid, day));
  const result = draftSchema.safeParse(value);
  if (!result.success) return null;
  if (now - new Date(result.data.updatedAt).getTime() > 24 * 60 * 60 * 1000) {
    await cacheDelete(key(user.uid, day));
    return null;
  }
  return result.data;
}
export async function saveWorkoutDraft(
  ...
  await cacheSet(
    key(user.uid, day),
    parseWorkoutDraft({ day, startedAt, updatedAt: new Date().toISOString(), entries }),
  );
}
export async function clearWorkoutDraft(user: User, day: DayKey): Promise<void> {
  await cacheDelete(key(user.uid, day));
}
