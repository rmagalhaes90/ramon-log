...
const backupDataSchema = z.object({
  workouts: userDataSchemas.workouts.optional(), bodyWeights: userDataSchemas.bodyWeights.optional(), bodyMeasurements: userDataSchemas.bodyMeasurements.optional(),
...
export function parseBackup(text: string): Backup {
  ...
  let value: unknown; try { value = JSON.parse(text); } catch { throw new Error('backupInvalid'); }
  const result = backupSchema.safeParse(value); if (!result.success) throw new Error('backupInvalid'); return result.data;
}

export async function createBackup(user: User): Promise<Backup> {
  const values = await Promise.all(keys.map(async(key) => [key, await loadUserData(user, key)] as const));
  return backupSchema.parse({ format: 'kyro-v4-backup', version: 1, exportedAt: new Date().toISOString(), data: Object.fromEntries(values.filter(([, value]) => value !== null)) });
}
...
export async function restoreBackup(user: User, backup: Backup): Promise<void> {
  ...
  try {
    for (const [key, value] of entries) await saveUserData(user, key, userDataSchemas[key].parse(value) as never);
  } catch (error) {
    await Promise.allSettled([...previous].filter((entry) => entry[1] !== null).map(([key, value]) => saveUserData(user, key, value as never)));
    throw error;
  }
}
