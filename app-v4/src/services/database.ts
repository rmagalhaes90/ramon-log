...
interface KyroDatabase extends DBSchema {
  ...
  photoQueue:{key:string;value:{id:string;uid:string;date:string;file:Blob;createdAt:number};indexes:{'by-uid':string}};
}
...
export function database(): Promise<IDBPDatabase<KyroDatabase>> {
  connection ??= openDB<KyroDatabase>('kyro-v4', 2, {
    upgrade(db,oldVersion) {
      if(oldVersion<1){db.createObjectStore('cache');const queue=db.createObjectStore('queue',{keyPath:'id'});queue.createIndex('by-next-attempt','nextAttemptAt');}
      if(oldVersion<2){const photos=db.createObjectStore('photoQueue',{keyPath:'id'});photos.createIndex('by-uid','uid');}
    },
  });
  return connection;
}
...
export async function pendingPhotoPut(value:{id:string;uid:string;date:string;file:Blob;createdAt:number}):Promise<void>{await(await database()).put('photoQueue',value);}
export async function pendingPhotos(uid:string){return(await database()).getAllFromIndex('photoQueue','by-uid',uid);}
export async function pendingPhotoDelete(id:string):Promise<void>{await(await database()).delete('photoQueue',id);}

export async function clearLocalData(): Promise<void> {
  const db = await database();
  const transaction = db.transaction(['cache', 'queue','photoQueue'], 'readwrite');
  await Promise.all([transaction.objectStore('cache').clear(), transaction.objectStore('queue').clear(),transaction.objectStore('photoQueue').clear(), transaction.done]);
}
