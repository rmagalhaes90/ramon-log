...
describe('backup and progress charts', () => {
  it('validates a strict versioned backup and rejects unknown payloads', () => {
    const valid = {
      ...
      data: { bodyWeights: [{ d: '2026-08-02', kg: 80 }] },
    };
    expect(parseBackup(JSON.stringify(valid)).data.bodyWeights).toHaveLength(1);
    expect(() => parseBackup(JSON.stringify({ ...valid, data: { admin: true } }))).toThrow(
      'backupInvalid',
    );
  ...
  it('produces bounded chronological SVG points without NaN', () => {
    expect(
      chartPoints(
        [
          { d: '2026-08-02', value: 80 },
          { d: '2026-08-01', value: 80 },
        ],
        100,
        50,
      ),
    ).toEqual([
      { d: '2026-08-01', value: 80, x: 0, y: 50 },
      { d: '2026-08-02', value: 80, x: 100, y: 50 },
    ]);
  });
});
