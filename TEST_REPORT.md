...
Resultados reais: typecheck strict conjunto (web, domínio e mobile), ESLint, **34 arquivos/97 testes web**, **1 arquivo/5 testes de domínio**, build Vite de produção e teste de integração do Firebase Auth Emulator passaram. O teste do Emulator criou uma conta, autenticou e excluiu a conta com sucesso. O build transformou 407 módulos; app JS 312,57 kB (89,21 kB gzip), CSS 30,83 kB (6,14 kB gzip), Auth 111,52 kB (33,26 kB gzip), Firestore 434,56 kB (128,87 kB gzip) e Storage 32,29 kB (10,63 kB gzip).

No Playwright, os smoke tests públicos passaram em Chromium e WebKit mobile (**2 passed**). Os quatro cenários autenticados foram ignorados pela configuração, conforme esperado sem a suíte completa de Emulators ativa; a operação básica de Auth foi coberta separadamente pelo teste de integração acima.

A checagem global de Prettier apontou somente `mobile/expo-env.d.ts`, arquivo gerado pelo Expo que já estava alterado localmente antes desta correção e não faz parte do commit. A versão rastreada desse arquivo e todos os arquivos desta fase passaram no Prettier. O fluxo Google/Apple em aparelho iOS/iPadOS real ainda precisa da confirmação manual após a publicação, pois o Emulator não reproduz o popup do provedor externo nem as políticas do Safari.

## Rodada 2026-08-08 — painel de diagnóstico de login alpha.61

A correção da alpha.60 (`signInWithRedirect` no lugar de `signInWithPopup`) não resolveu o login no iPad — usuário reporta que agora nenhuma mensagem de erro aparece, mas o login continua não completando. Sem acesso a um iPhone/iPad real neste ambiente para depurar diretamente, e sendo esse tipo de aparelho difícil de inspecionar via console do navegador sem um Mac por perto, o usuário pediu explicitamente por uma forma de ver o que está acontecendo.

Implementado um painel de diagnóstico temporário, visível diretamente na tela de login (`app-v4/src/core/authDebug.ts` + instrumentação em `features/auth/index.ts` + renderização em `main.ts`):

- Grava em `localStorage` (não `sessionStorage` — precisa sobreviver à navegação de ida e volta do redirecionamento) cada evento relevante: inicialização do módulo de auth (com `userAgent`, se está rodando como app instalado via "Adicionar à Tela de Início" no iOS, status online, e um teste real de leitura/escrita em `localStorage`/`sessionStorage`), início do redirecionamento (`signInWithRedirect`) pro Google/Apple, o momento de checar o resultado do redirecionamento ao voltar (`getRedirectResult`) com o resultado exato (sucesso/vazio/erro com código e mensagem), e cada mudança de estado do `onAuthStateChanged` (com email e providers).
- Renderizado como uma seção visível (não escondida atrás de um menu) logo abaixo do cartão de login, com o registro em texto monoespaçado e um botão "Copiar diagnóstico" (usa a Clipboard API, com mensagem de fallback caso não tenha permissão) e "Limpar".
- Hipótese mais provável a confirmar com esses dados reais: um PWA instalado via "Adicionar à Tela de Início" no iOS roda num contexto de armazenamento (`localStorage`/cookies) historicamente separado do Safari normal — se a viagem de ida e volta pro login do Google passar pelo Safari do sistema e voltar pro contexto standalone do PWA, o estado que o Firebase Auth precisa pra reconhecer o redirecionamento como concluído pode se perder no meio do caminho, explicando "nenhum erro, mas também não loga" (nem sucesso nem falha são reportados porque o SDK nunca reconhece que havia um redirecionamento pendente).

Durante a implementação, um erro real de tipo apareceu no build (não no `typecheck` isolado, por causa de um cache incremental do `tsc -b` que não invalidou corretamente entre as duas execuções): `exactOptionalPropertyTypes` rejeitava atribuir `undefined` explicitamente a um campo opcional. Corrigido construindo a entrada do log condicionalmente em vez de sempre incluir a chave `detail`. Também corrigido um `ReferenceError: window is not defined` que quebrava `tests/auth.test.ts` — a função de snapshot do ambiente acessava `window.matchMedia` sem checar se `window` existe, o que crasha em ambientes de teste não-navegador; agora tem uma guarda no topo da função e cada bloco sensível a armazenamento já estava em try/catch.

Verificado ao vivo (navegador local, sem emulador necessário pra esta parte): o painel aparece na tela de login com o texto "Diagnóstico de login", mostra entradas reais (`module:init`, `redirect:checking`, `onAuthStateChanged`, `redirect:result`) com timestamps e detalhes em JSON legível; o botão "Copiar diagnóstico" tenta a Clipboard API e mostra a mensagem de fallback corretamente quando a permissão não está disponível (esperado neste ambiente de teste); o botão "Limpar" apaga o registro e a tela volta a mostrar "Nenhum evento registrado ainda.". Não é possível validar a causa raiz suspeita (contexto de armazenamento do PWA no iOS) sem os dados reais do aparelho do usuário — esse é exatamente o próximo passo, pedir pra ele tentar o login de novo e copiar o texto do painel.

Bateria completa: `typecheck`, `lint`, `format:check` (só `mobile/expo-env.d.ts` pré-existente), **34 arquivos/99 testes Vitest** e `build`, todos com código 0 (rodados uma segunda vez do zero depois do problema de cache do `tsc -b`, pra garantir que não havia mais nada escondido). Versão sincronizada para `4.0.0-alpha.61`.
...
**1. Confete a cada série concluída.** Pedido do usuário: a animação (confete + selo de "novo recorde") não deveria disparar toda vez que uma série é marcada como concluída — só faz sentido no fim do treino completo, que já tem sua própria celebração desde a alpha.38. Ao investigar o código, a condição de "novo recorde" (`!record || ...`) contava QUALQUER exercício ainda sem nenhum registro salvo em `exerciseRecords` como recorde automático — na prática, isso cobre a maioria das séries de exercícios não totalmente consolidados no histórico, explicando a frequência alta reportada. Removida a chamada da celebração no toggle de série concluída (junto com a função `celebratePersonalRecord`, o CSS `.pr-badge` e a chave de tradução, todos órfãos depois da remoção); a celebração de fim de treino (`spawnConfetti` chamado por `showCelebration`) continua intacta.

**2. Botão de remover "vazando" atrás da série concluída.** Reproduzido e confirmado via hit-test real (`document.elementFromPoint`) no navegador: uma série marcada como concluída deixava o botão vermelho de remover virar clicável/visível mesmo sem swipe. Causa raiz: `.set-row.done` (e a variante mais específica `.set-row-wrap .set-row.done`) usa um fundo verde quase transparente (`#d7ff3d0a`, ~4% de opacidade) só pra dar um destaque sutil — isso sempre existiu, mas só passou a ter efeito colateral visível depois do conserto de z-index da alpha.57, que fez a linha da série ficar **por cima** do botão de remover (antes disso o botão sempre aparecia por cima de qualquer forma, mascarando esse problema específico). Com a linha agora quase transparente quando concluída, o vermelho por baixo aparecia através dela. Corrigido trocando por uma cor sólida (`color-mix(in srgb, var(--panel) 96%, var(--volt) 4%)`) que preserva o mesmo tom visual sem deixar nada por baixo aparecer, nos dois lugares onde a regra existia (e que também se adapta automaticamente ao tema claro/escuro, diferente do hex fixo anterior).

**3. Sem feedback visual nos botões de administração.** Clicar em "Bloquear", "Tornar admin" ou "Tornar coach" na tela de gerenciar usuários não dava nenhuma indicação de que algo estava acontecendo até a lista atualizar sozinha. Adicionado estado de carregamento: o botão clicado (e os outros da mesma linha, pra evitar cliques duplos) desabilita e mostra "Aguarde…" imediatamente; se a ação falhar, volta ao texto e estado originais automaticamente.

Verificado ao vivo no emulador Auth+Firestore: (1) marcar uma série como concluída com carga/reps altos não gerou nenhum elemento `.confetti-burst` nem `.pr-badge`; (2) hit-test no botão de remover de uma série concluída confirmou que ele fica escondido atrás da linha (cor de fundo sólida `color(srgb 0.12 0.13 0.12)`, não mais transparente) até o swipe real acontecer; (3) clicar em "Tornar admin" (usando a conta bootstrap `rmagalhaes90@gmail.com`, criada só no emulador isolado pra este teste) mostrou o botão desabilitado com texto "Aguarde…" imediatamente após o clique, e — como o emulador de Functions não estava rodando nesta verificação — a chamada falhou como esperado e o botão corretamente voltou ao texto/estado original em vez de ficar travado, confirmando também o caminho de erro.

Bateria completa: `typecheck`, `lint` (inclui `functions/`), `format:check` (só `mobile/expo-env.d.ts` pré-existente), **34 arquivos/99 testes Vitest** e `build`, todos com código 0. Versão sincronizada para `4.0.0-alpha.59`.
...
**1. "Remover" do exercício continuava visível o tempo inteiro, mesmo depois da alpha.56.** Investigação: o teste de swipe feito durante a verificação da alpha.56 só checava sobreposição de retângulos (`getBoundingClientRect`), não o resultado real de pintura/z-order — um falso positivo. Reproduzido de forma definitiva injetando a marcação real da alpha.56 numa aba do site publicado e usando `document.elementFromPoint` (hit-test real do navegador) no centro do botão vermelho: o clique caía no botão de remover mesmo sem nenhum swipe. Causa raiz: tanto `.exercise-top-delete` quanto `.set-row-delete` (esse último existe desde a alpha.53) são posicionados com `position:absolute`, e o cabeçalho/linha que devia cobri-los é só `position:relative` sem `z-index` — como os dois entram na mesma "camada" (z-index automático), o navegador desempata por ordem no HTML, e como o botão vermelho é inserido depois do cabeçalho no código, ele sempre pintava por cima, nunca esteve de fato escondido. Esse é o MESMO bug do recurso original de remover série (alpha.53) — nunca foi notado porque a faixa vermelha de 76px é menos chamativa numa linha de série estreita do que no cabeçalho largo do exercício. Corrigido dando `z-index:1` ao cabeçalho/linha (a camada de cima, que desliza), garantindo que ele sempre cubra o botão por baixo até o swipe empurrá-lo pro lado. Confirmado via o mesmo hit-test real: escondido antes do swipe, clicável depois, nos dois padrões (série e exercício).

**2. "Continuar com Google" só mostrava texto, sem o ícone.** Nunca teve ícone nenhum implementado desde que o botão existe — confirmado lendo o HTML gerado (`<button id="google">texto puro</button>`). Adicionado o logo oficial de quatro cores do Google via SVG inline, e um ícone da Apple também no botão correspondente.

**3. Cadastro de conta nova falhando com "Não foi possível autenticar agora".** Não foi possível reproduzir diretamente contra produção (política interna: nunca crio contas reais nem digito senhas em formulários de autenticação, nem para diagnóstico — isso vale mesmo quando autorizado). Investigação alternativa: exportado (somente leitura, via `firebase auth:export`) a lista de contas reais do Firebase Authentication em produção — encontrada uma conta criada nesse mesmo dia, nunca mais logada depois da criação, com email já verificado apesar disso (indício de que o link de verificação foi aberto automaticamacamente por um scanner de email, não pelo usuário revisitando manualmente). Isso bate exatamente com um bug real em `createAccount()`: a conta é criada com sucesso primeiro, e só DEPOIS o app tenta enviar o email de verificação — se esse envio falhar por qualquer motivo transitório (rede, limite de envio), o cadastro inteiro era reportado como erro genérico, mesmo a conta já existindo de verdade, sem deixar o usuário chegar na tela de "verifique seu email". Corrigido: falha ao enviar o email de verificação não trava mais o cadastro (a conta germina como signed-in, `onAuthStateChanged` leva pra tela de verificação normalmente, que já tem botão de reenviar). Também adicionada uma mensagem específica ("este email já tem uma conta, tente entrar ou 'esqueci a senha'") para `auth/email-already-in-use`, que antes caía no mesmo erro genérico. A conta específica encontrada travada já está com email verificado — o usuário consegue acessá-la direto por "Esqueci a senha" sem precisar de nenhuma mudança de código.

**4. Contas na lista de "gerenciar usuários" que não existem mais no Firebase.** A auto-cura da alpha.56 (`getAuthUserOrCleanStale`) só limpa uma conta órfã quando alguém tenta uma ação nela especificamente — contas nunca clicadas ficavam na lista pra sempre. Nova Cloud Function `cleanupOrphanedSharedUsers` (admin) varre toda a coleção `sharedUsers` de uma vez, verifica cada uma contra o Authentication e apaga as que não existem mais lá; novo botão "Limpar contas órfãs" na tela de administração, com confirmação e contagem do resultado.

**5. Duplicata ao adicionar exercício.** Tela "+ Adicionar exercício" agora verifica se o exercício já está na rotina do dia selecionado (por nome) e, se estiver, mostra o botão como "Já na rotina" desabilitado em vez de "Adicionar", prevenindo duplicata sem precisar lembrar manualmente o que já foi incluído.

Bateria completa: `typecheck`, `lint` (inclui `functions/`), `format:check` (só `mobile/expo-env.d.ts` pré-existente), **34 arquivos/99 testes Vitest** e `build`, todos com código 0. Verificação visual do bug de z-index feita via hit-test real (`elementFromPoint`) contra o CSS carregado tanto em produção (antes do fix) quanto localmente (depois do fix), nos dois padrões de swipe. Ícone do Google confirmado renderizando via inspeção do DOM. Os itens 3 e 4 não puderam ser testados ponta-a-ponta (accão real de autenticação/Cloud Function em produção está fora do que posso executar diretamente) — cobertos por investigação forense dos dados reais de produção (somente leitura) e revisão de código. Versão sincronizada para `4.0.0-alpha.57`.

## Rodada 2026-08-07 — sete ajustes de uso real alpha.56

Usuário testou a alpha.55 ao vivo e reportou sete pontos na mesma rodada:

1. "Remover exercício fica visível o tempo inteiro, quero estilo swipe pra esquerda." O botão "Remover" do cabeçalho do exercício (Treino e Rotina) saiu de um botão sempre visível para o mesmo gesto de arrastar-para-esquerda já usado nas séries individuais — extraído o helper compartilhado `attachSwipeToDelete` (Pointer Events, funciona em toque e mouse) reaproveitado pelas séries e agora também pelo cabeçalho do exercício.
2. "Lista de gerenciar usuários não reflete o banco — deletei direto e não sumiu; adicionar admin dá erro genérico." Causa raiz dupla: (a) `listSharedUsers()` usava `getDocs()`, que pode servir do cache local do Firestore em vez de ir ao servidor — trocado para `getDocsFromServer()`; (b) contas apagadas direto do Firebase Auth (sem passar pela função de exclusão do app) deixavam um documento órfão em `sharedUsers`, e tentar promover/bloquear esse uid batia num `auth/user-not-found` cru, reportado ao usuário como o toast genérico "Algo deu errado". Novo helper `getAuthUserOrCleanStale` no backend detecta esse caso, apaga o documento órfão sozinho e devolve um erro `not-found` específico; o cliente detecta esse código e recarrega a lista automaticamente em vez de mostrar o toast genérico.
3. "RIR e RPE podem ser opcionais, maioria não vai usar no início." Novo toggle em Configurações ("Mostrar RIR e RPE no treino"), desligado por padrão — com o toggle desligado, as duas colunas somem da grade de séries (`layout` CSS compacto com 2 colunas em vez de 4) tanto na tela de Treino quanto na de Rotina.
4. "PR pode ser automático, tipo antes tinha animação de troféu." Marcar uma série como concluída agora compara a carga e o 1RM estimado com o recorde já salvo daquele exercício (`exerciseRecords`, já existente); se for um novo recorde, dispara confete + um selo "🏆 Novo recorde pessoal!" na hora, distinto da celebração de fim de treino inteiro.
5. "Checkbox pra repetir a 1ª série pra todas, facilita a vida." Novo checkbox "Repetir carga/reps da 1ª série em todas", visível só em exercícios com mais de uma série; ao marcar, copia carga e reps da primeira série pras demais de uma vez.
6. - 7. "Linkar exemplos em inglês nos exercícios, e o GIF não pode ficar rodando sozinho — clicar pra ver, igual o vídeo do YouTube." Duas partes: o GIF deixou de ser um `<img>` sempre carregado/animado dentro do card do exercício e virou um botão "Ver demonstração" que abre um modal (mesmo padrão visual do modal de vídeo do YouTube já existente). E foi criada uma ferramenta de vínculo em massa no Gerenciador de exercícios ("Vincular exemplos em massa") — traduz cada nome em português do catálogo (~180 exercícios) para inglês via um dicionário palavra-por-palavra construído a partir do vocabulário real do catálogo (`translateExerciseName.ts`, testado isoladamente), busca o termo traduzido na API paga e vincula automaticamente o primeiro resultado, salvando em lotes de 20; é uma ferramenta opcional, disparada manualmente pelo admin (consome cota da API a cada execução), não rodada automaticamente.

Verificado ao vivo no navegador contra o emulador de Auth+Firestore (cadastro, verificação de e-mail, onboarding, gerador automático de treino "Peito", aplicar rotina): (1) swipe simulado por Pointer Events no cabeçalho do exercício revelou o botão e removeu o exercício certo; (2) toggle de RIR/RPE confirmado nos dois sentidos (3 campos por série quando oculto, 5 quando visível, sem a classe `compact`); (3) checkbox de repetir 1ª série copiou carga/reps pras 4 séries do exercício testado; (4) preencher uma série com carga alta e marcar como concluída disparou confete e o selo de recorde imediatamente. Os itens 2 (auto-cura da lista de admin) e 6/7 (clique-pra-ver e ferramenta de vínculo em massa) foram cobertos por teste unitário/schema e revisão de código, mas não pelo navegador nesta rodada — o primeiro exige um deploy de produção das Cloud Functions pra fazer efeito, e a ferramenta de vínculo em massa não deveria ser disparada durante teste (gastaria cota real da API).

Bateria completa: `typecheck`, `lint` (inclui `functions/`), `format:check` (só `mobile/expo-env.d.ts` pré-existente), **34 arquivos/99 testes Vitest** e `build`, todos com código 0. Versão sincronizada para `4.0.0-alpha.56`.
